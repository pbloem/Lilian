...
public class Series
{
	...
	/**
	 * Returns a list of ranges correspondong to the given series. For instance
	 * the series (1, 2, 3, 4) would generate the ranges (1:2, 2:3, 3:4). 
	 *  
	 * @return
	 */	
	public static <T extends Number> List<Range> ranges(List<T> series)
	{
		return ranges(series, false);
	}	
	
	/**
	 * Returns a list of ranges correspondong to the given series. For instance
	 * the series (1, 2, 3, 4) would generate the ranges 
	 * (-inf:1, 1:2, 2:3, 3:4, 4:inf). 
	 *  
	 * @param includeEdges Whether to include ranges including infinity at the 
	 *        beginning and end
	 * @return
	 */
	public static <T extends Number> List<Range> ranges(List<T> series, boolean includeEdges)
	{
		if(includeEdges && series.size() == 0)
			throw new IllegalArgumentException("series ("+series+") must not be empty when includeEdges is true");
		if(includeEdges && series.size() < 2)
			throw new IllegalArgumentException("series ("+series+") must contain at least two elements when includeEdges is true");
		
		List<Range> ranges = new ArrayList<Range>(series.size() + (includeEdges ? 2 : 0));
		
		if(includeEdges)
			ranges.add(new Range(
					Double.NEGATIVE_INFINITY, 
					series.get(0).doubleValue()));
		
		Iterator<T> numberIterator = series.iterator();
		double first = numberIterator.next().doubleValue(),
		       second;
		
		while(numberIterator.hasNext())
		{
			second =  numberIterator.next().doubleValue();
		...
		if(includeEdges)
			ranges.add(new Range(
					series.get(series.size() - 1).doubleValue(),
