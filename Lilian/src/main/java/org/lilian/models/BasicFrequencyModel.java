...
public class BasicFrequencyModel<T> extends AbstractFrequencyModel<T>
	 ...
	 */	
	public BasicFrequencyModel(BasicFrequencyModel<T> model)
	{
	 ...
	 * given set of tokens.
	 */
	public BasicFrequencyModel(FrequencyModel<T> model, Collection<T> tokens)
	{
		this();
		for(T token : tokens)
	...
	public BasicFrequencyModel(Collection<T> corpus)
	{
		add(corpus);
	}
