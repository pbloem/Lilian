...
public class TrieMarkovModel<T> extends AbstractMarkovModel<T>
{
	...
	{
		totals.set(token.size()-1, 
				totals.get(token.size()-1) + 1);
...
//	 *
//	 */
//	public double conditionalEntropy(List<T> given)
//	{
...
//		}		
		...
		public double numAncestors(int depth)
		{
			...
			for(Node child : children.values())
				sum += child.numAncestors(depth - 1); 
			
			return sum;			
		}
		
		public void tokens(int depth, List<T> ngram, List<List<T>> list) 
		{
			if(depth == 0)
			{
				list.add(ngram);
			...
			for(T token : children.keySet())
			{
				Node child = children.get(token);
				
				List<T> next = new ArrayList<T>(ngram.size() + 1);
				next.addAll(ngram);
				next.add(token);
				
				child.tokens(depth - 1, next, list);
			}
		}
	}
	...
	{
		List<List<T>> list = new ArrayList<List<T>>( (int)model(order).distinct() );
		root.tokens(order, new ArrayList<T>(), list);
		return list;
	}
}
