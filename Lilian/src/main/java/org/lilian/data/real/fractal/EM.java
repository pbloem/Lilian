 ...
 * by a d-length k-ary code. By analogy with the EM algorithm for MOG models:
 * If we know the transformations of the IFS, we can find the codes for the data
 * points. If we know which component each data point came from (and its
 * code), we can find the transformations. The solution, again, is to start with
 * an arbitrary model, and iterate the steps.
 * </p>
 ...
 * </p>
 * How do we find the transformations given the codes? We look for point pairs 
 * with codes like a=1012220 and b=012220?. Since b is a left-shifted version 
 * of a, we know that applying transformation 1 to b will yield a (with an error 
 * that decreases with the depth). We collect all such points for each 
 * transformation (and at all depths) and find the optimal mapping between them.
 * </p>
 * <h2>Details:</h2>
 * <ul>
 ...
 * points and map specific points drawn from the standard MVN to this MVN. We
 * do the same for the matching code. </li>
 * <li>
 * For a proper IFS model each transformation also has a weight. We find this 
 * by comparing the sizes (in number of points) of matching code pairs. (see 
 * the code for the details).</li>
 * <li>
...
public class EM implements Serializable
{
	...
	//   perturbed
	public static final double CONTRACTION_THRESHOLD = 1E-12;
	 ...
	 */
	public EM(
			IFS<Similitude> initial, List<Point> data,
			double perturbVar, boolean useSphericalMVN)
	{
		...
		if(dimension != data.get(0).dimensionality())
			throw new IllegalArgumentException("Data dimension ("+data.get(0).dimensionality()+") must match initial model argument ("+dimension+")");
	...
	protected void codesToModel()
	{
		...
		BasicFrequencyModel<Integer> priors = 
		...
		for(int i : Series.series(numComponents))
		{
			...
			if(n != 0) // codes found containing this comp
			{
				...
				Similitude map = 
					org.lilian.data.real.Maps.findMap(maps.from(i), maps.to(i));
				// * Find the weight for the frequency pairs
				double weight = findScalar(maps.fromWeights(i), maps.toWeights(i));
				
				// * If the map contracts too much, we perturb it slightly
				double det = MatrixTools.getDeterminant(map.getTransformation());
				if(Math.abs(det) < CONTRACTION_THRESHOLD || Double.isNaN(det))
					map = Parameters.perturb(map, 
				...
				assigned.add(i);
			} else { // No codes found with this component
				unassigned.add(i);
			}
		}
	 ...
	 * define the map. The points span mvn distributions of subsets of the 
	 * data (as defined by codes of varying length).
	 *  
	 * @return
	 */
	...
	protected class Node implements Serializable
	{
		 ...
		 * child.
		 * 
		 * @param codeSuffix The suffix of the code after this node
		 * @param point The point to be observed.
		 */
		public void observe(List<Integer> codeSuffix, Point point)
		{
			...
			children.get(symbol).observe(codeSuffix.subList(1, codeSuffix.size()), point);
		}
		 ...
		 * leaf if it is at the set maximum depth for this iteration of the EM 
		 * algorithm. It may be that is has no children yet but they will be 
		 * created by the observation of a future code.
		 */
		...
		/**
		 * A multivariate normal distribution fitted to the points stored at this node
		 * 
		...
		public MVN mvn()
		{
			if(mvn == null)
				try {
					mvn = useSphericalMVN ? MVN.findSpherical(points) : MVN.find(points);
		...
		/**
		 * Returns the node for the given code (suffix) (starting from this node).
		 * 
		 * @param code The code suffix for which to find the node starting from 
		 * the current node.
		...
		public void findPairs(Maps maps)
		{
			...
			if(code().size() > 0) // * Execute for this node
			{
				...
				if(nodeFrom != null)
				{
					...
					{
						if(m < COVARIANCE_THRESHOLD) // Not enough points to consider covariance
						{
							for(int i : series(m))
						...
						} else
						{
							...
							//  points)
							List<Point> points = new MVN(dimension).generate(points().size());
	...
	protected class Maps
	{
		 ...
		 * 
		 * @param component The component that maps from the first to the second point
		 ...
		 * 
		 * @param component The component that maps from the first to the second frequency
		 * @param from The from frequency
		 * @param to The to frequency
		 */		
